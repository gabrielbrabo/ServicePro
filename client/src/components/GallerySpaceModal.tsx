...
// Modal de compra de ESPACO de galeria (armazenamento). O dono paga um pacote
// que amplia o total de fotos que pode guardar (single=1, antes/depois=2).
// Cobra na conta da plataforma (receita da empresa), sem split. PIX ou cartao.
export function GallerySpaceModal({
  establishmentId,
  space,
  onClose,
  onPurchased,
  ...
  const cycleLabel = space.billingCycle === "anual" ? "ano" : "mês";
  const chargeNow = space.packChargeNowCents;
  ...
  const startExtra = useRef(space.extraSlots);
  ...
  const checkNow = async () => {
    setChecking(true);
    try {
      const st = await subscriptionApi.gallery(establishmentId);
      if (st.extraSlots > startExtra.current) {
        stopPoll();
        markGranted();
      }
    } catch {
  ...
  const comprar = async () => {
    ...
    setLoading(true);
    try {
      const res = await subscriptionApi.buyGallery(establishmentId, payload);
      if (res.granted) {
        markGranted();
        return;
      }
        ...
        <div className="flex items-start justify-between gap-3 border-b border-ink/10 p-5">
          <div>
            <h2 className="font-display text-lg font-bold text-ink">
              Adicionar espaço de galeria
            </h2>
            <p className="mt-0.5 text-sm text-ink/60">
              Cada pacote adiciona {space.packSlots} espaços (= {space.packSlots}{" "}
              fotos normais ou {Math.floor(space.packSlots / 2)} antes e depois).
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-5">
          <div className="rounded-xl bg-sand/50 px-4 py-3 text-center">
            ...
            <p className="mt-0.5 text-xs text-ink/50">
              Depois, {brl(space.packPriceCents)}/{cycleLabel} junto da sua
              assinatura.
            </p>
          </div>
          ...
          {granted ? (
            <div className="rounded-xl bg-teal-50 px-4 py-4 text-center">
              ...
              <p className="mt-1 text-sm text-teal-700/80">
                Agora você já pode publicar mais fotos.
              </p>
              <button
                onClick={onClose}
            ...
            <div className="space-y-3">
              ...
              <p className="text-center text-xs text-ink/50">
                Assim que o pagamento cair, o espaço é liberado automaticamente.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
