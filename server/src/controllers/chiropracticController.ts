...
import { ChiropracticProfile } from "../models/ChiropracticProfile";
import { ChiropracticSession } from "../models/ChiropracticSession";
...
export const getProfile = async (
  ...
  try {
    ...
    const profile = await ChiropracticProfile.findOne({
      establishment: establishmentId,
      client: clientId,
    });
    if (!profile) {
...
// ============ ATENDIMENTOS (avaliacao postural + ajustes) ============

const sanitizePosture = (raw: unknown) => {
  const p = (raw || {}) as Record<string, unknown>;
  return {
    head: cleanText(p.head),
    shoulders: cleanText(p.shoulders),
    pelvis: cleanText(p.pelvis),
    cervical: cleanText(p.cervical),
    thoracic: cleanText(p.thoracic),
    lumbar: cleanText(p.lumbar),
    scoliosis: cleanText(p.scoliosis),
    notes: cleanText(p.notes),
  };
};

const sanitizeAdjustments = (raw: unknown) =>
  Array.isArray(raw)
    ? (raw as unknown[])
        .map((a) => {
          const it = (a || {}) as Record<string, unknown>;
          return {
            segment: cleanText(it.segment),
            technique: cleanText(it.technique),
...
const buildSessionData = (body: Record<string, unknown>) => {
  const data: Record<string, unknown> = {
    ...
    posture: sanitizePosture(body.posture),
    posturePhotos: sanitizePhotos(body.posturePhotos),
    adjustments: sanitizeAdjustments(body.adjustments),
...
async function loadSession(
  ...
  return ChiropracticSession.findOne({
    _id: sessionId,
    establishment: establishmentId,
    client: clientId,
  });
...
export const listSessions = async (
  ...
  try {
    const { establishmentId, clientId } = req.params;
    if (!(await canManage(establishmentId, req.userId))) return void deny(res);
    const items = await ChiropracticSession.find({
      establishment: establishmentId,
      client: clientId,
    })
...
export const createSession = async (
  ...
  try {
    ...
    const item = await ChiropracticSession.create({
      establishment: establishmentId,
      client: clientId,
      author: req.userId,
